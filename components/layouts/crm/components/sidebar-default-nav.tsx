...
import { AccordionMenu, AccordionMenuItem } from '@/components/ui/accordion-menu';
...
function MoreDropdownMenu({ item }: { item: NavItem }) {
    const { isSidebarNavItemPinned, unpinSidebarNavItem, pinSidebarNavItem, sidebarCollapse, getSidebarNavItems } =
        useLayout();
        ...
        <>
            ...
            {(item.more || item.new) && (
                <div className="opacity-0 flex items-center gap-1 group-hover:opacity-100 [&:has([data-state=open])]:opacity-100">
                    {item.more && <>{item.id === 'tasks' && <TasksDropdownMenu trigger={trigger} />}</>}
                    {item.new && (
                        <Tooltip delayDuration={500}>
                            <TooltipTrigger asChild>
            ...
            >
                {filteredNavItems.map((item) => (
                    <AccordionMenuItem key={item.id} asChild value={item.path || item.id}>
                        <div>{sidebarCollapse ? <NavItemCollapsed item={item} /> : <NavItem item={item} />}</div>
                    </AccordionMenuItem>
                ))}
            </AccordionMenu>
        </div>
