...
const WorkExperience = () => {
    ...
    const renderItem = (item: IWorkExperienceItem, index: number) => {
        return (
            <div key={index}>
                {item.heading ? (
                    <div className="text-secondary-foreground font-semibold text-sm leading-none">{item.heading}</div>
                ) : (
                    <div className="flex align-start gap-3.5">
                        {item.image && (
                            <img src={toAbsoluteUrl(`/media/brand-logos/${item.image}`)} className="h-9" alt="image" />
                        )}
                        <div className="flex flex-col gap-1">
                            ...
                            {item.desc && <span className="text-sm font-medium text-mono">{item.desc}</span>}
                            {item.date && (
                                <span className="text-xs text-secondary-foreground leading-none">{item.date}</span>
                            )}
                        </div>
                    </div>
                )}
