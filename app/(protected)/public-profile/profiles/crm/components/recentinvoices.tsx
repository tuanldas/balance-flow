...
import { Barcode, EllipsisVertical, LucideIcon, Scroll, ShoppingBag, Tag, TicketPercent } from 'lucide-react';
            ...
            <div key={index} className="flex justify-between gap-3">
                <div className="flex items-center gap-2.5">
                    <div className="flex items-center justify-center size-7.5 shrink-0 bg-accent/60 rounded-lg border border-input">
                        <item.icon className="text-base text-secondary-foreground" size={16} />
                    </div>
                    <div className="flex flex-col gap-0.5">
                        <span className="text-sm font-medium text-mono cursor-pointer hover:text-primary mb-px">
                            {item.number}
                        </span>
                        <span className="text-xs text-secondary-foreground">{item.date}</span>
                    </div>
                </div>
                <div className="flex items-center gap-2.5">
