...
const Likes = () => {
    ...
    const toggleConnection = (index: number) => {
        setItems((prevItems) =>
            prevItems.map((item, i) => (i === index ? { ...item, connected: !item.connected } : item)),
        );
    };
            ...
            <div key={index} className="flex items-center flex-wrap gap-2">
                <div className="flex items-center grow gap-2.5">
                    ...
                    <div className="flex flex-col">
                        ...
                        <span className="text-sm text-secondary-foreground">{item.subscribers} mutual subscribers</span>
                    </div>
                </div>
                <Button variant={item.connected ? 'outline' : 'primary'} onClick={() => toggleConnection(index)}>
                    {item.connected ? (
                        <>
                            <CircleCheck /> Connect
                        </>
