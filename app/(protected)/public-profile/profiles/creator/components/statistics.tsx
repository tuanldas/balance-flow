        ...
        return (
            <React.Fragment key={index}>
                <div className="flex flex-col items-center gap-1.5">
                    <span className="text-mono text-2xl lg:text-2xl font-semibold">{item.value}</span>
                    <span className="text-secondary-foreground text-sm font-normal">{item.title}</span>
                </div>
                {index === 0 && <span className="sm:ms-8 sm:ps-8 border-s border-s-border"></span>}
            </React.Fragment>
        );
    };
