...
import { RoleSchema, RoleSchemaType } from '@/app/(protected)/user-management/roles/forms/role-schema';
...
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
    try {
        ...
        const permissions = role.permissions.map((p: UserRolePermission) => p.permission);
...
export async function PUT(request: NextRequest, context: { params: Promise<{ id: string }> }) {
    try {
        ...
        const { name, slug, description, permissions }: RoleSchemaType = parsedData.data;

        // Check uniqueness for name and slug
        const isUniqueRole = await isUnique('userRole', { slug, name }, { id });
        if (!isUniqueRole) {
            return NextResponse.json({ message: 'Name or slug must be unique' }, { status: 400 });
        }
...
export async function DELETE(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
    try {
        ...
        if (role.isProtected) {
            return NextResponse.json({ message: 'Cannot delete a protected role' }, { status: 403 });
        }
        ...
        if (roleAssignedToUsers > 0) {
            return NextResponse.json({ message: 'Role is assigned to users and cannot be deleted' }, { status: 403 });
        }
