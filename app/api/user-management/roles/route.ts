...
export async function POST(request: NextRequest) {
    try {
        ...
        if (!isUniqueRole) {
            return NextResponse.json({ message: 'Name and slug must be unique' }, { status: 400 });
        }

        // Use a Prisma transaction to ensure all operations succeed or fail together
        const createdRole = await prisma.$transaction(async (tx: Prisma.TransactionClient) => {
            // Create the new role
            ...
            if (permissions && permissions.length > 0) {
                const rolePermissionEntries = permissions.map((permissionId: string) => ({
                    roleId: newRole.id,
                    permissionId,
                }));
