...
export async function POST(request: NextRequest) {
    try {
        ...
        if (logoAction === 'save' && logoFile && logoFile instanceof File && logoFile.size > 0) {
            try {
                // Create a new File from the existing one
                const fileCompatible: File = new File([await logoFile.arrayBuffer()], logoFile.name, {
                    type: logoFile.type,
                });
        ...
        await prisma.systemSetting.update({
            ...
            data: {
                ...
                currency,
                currencyFormat,
                logo: logoAction === 'remove' ? null : logoAction === 'save' ? logoUrl : undefined,
            },
        });
        ...
        return NextResponse.json(
            {
                message: "Oops! Something didn't go as planned. Please try again in a moment." + error,
            },
            { status: 500 },
        );
    }
