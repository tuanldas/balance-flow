...
interface IProjectExtendedProps {
    status: {
        variant?: 'primary' | 'destructive' | 'secondary' | 'info' | 'success' | 'warning' | null | undefined;
        label: string;
    };
    logo: string;
    title: string;
        ...
        <Card className="overflow-hidden grow justify-between">
            <div className="p-5 mb-5">
                ...
                <div className="flex justify-center mb-2">
                    <img src={toAbsoluteUrl(`/media/brand-logos/${logo}`)} className="min-w-12 shrink-0" alt="image" />
                </div>
                <div className="text-center mb-7">
                    <Link href={url} className="text-lg font-medium text-mono hover:text-primary">
                        {title}
                    </Link>
