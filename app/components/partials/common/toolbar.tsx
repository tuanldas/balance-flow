...
const Toolbar = ({ children }: { children: ReactNode }) => {
    return <div className="flex flex-wrap items-center lg:items-end justify-between gap-5 pb-7.5">{children}</div>;
};
...
const ToolbarPageTitle = ({ text }: { text?: string }) => {
    ...
    return <h1 className="text-xl font-medium leading-none text-mono">{text ?? item?.title}</h1>;
};

const ToolbarDescription = ({ children }: { children: ReactNode }) => {
    return <div className="flex items-center gap-2 text-sm font-normal text-secondary-foreground">{children}</div>;
};
