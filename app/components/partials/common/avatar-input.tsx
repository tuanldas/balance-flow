...
export function AvatarInput() {
    const [avatar, setAvatar] = useState<ImageInputFile[]>([{ dataURL: toAbsoluteUrl(`/media/avatars/300-2.png`) }]);

    return (
        <ImageInput value={avatar} onChange={(selectedAvatar) => setAvatar(selectedAvatar)}>
            {({ onImageUpload }) => (
                <div className="size-16 relative cursor-pointer" onClick={onImageUpload}>
                    <TooltipProvider>
                        <Tooltip>
